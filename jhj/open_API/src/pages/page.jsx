...
export default function Page() {
  ...
  const {
    ...
    queryKey: ["games", searchKeyword],
    queryFn: () => {
      if (searchKeyword.trim()) {
        return getSearchGames(searchKeyword);
      }
  ...
  const {
    ...
    queryKey: ["gameDetail", selectedGameId],
    queryFn: () => getGameDetail(selectedGameId),
  ...
  return (
    <div className="min-h-screen bg-blue-100 p-6 text-gray-900">
      ...
      <p className="mb-6">
        게임 이름을 검색하면 게임 정보와 상세 설명을 확인할 수 있어요!
      </p>
      ...
      <section className="mb-6 rounded-xl bg-white p-4 shadow-xl">
        ...
        <input
          ...
          className="mr-2 rounded border border-blue-500 px-3 py-2"
        />

        <button
          onClick={handleSearch}
          className="mr-2 rounded-xl bg-blue-500 px-4 py-2 font-bold text-white hover:bg-blue-600"
        >
          검색
        </button>

        <button
          onClick={handlePopularGames}
      ...
      <section className="mb-6 rounded-xl bg-white p-4 shadow-xl">
        ...
        <p>검색어: {searchKeyword || "인기 게임"}</p>
        <p>검색 결과 수: {games.length}개</p>
      </section>

      {selectedGameId && (
        <section className="mb-6 rounded-lg bg-white p-4 shadow-xl">
          <h2 className="mb-3 text-xl font-bold">게임 상세 정보</h2>

          {isDetailLoading && <p>상세 정보를 불러오는 중...</p>}

          {isDetailError && <p>게임 상세 정보를 불러오지 못했습니다.</p>}
      ...
      <section className="rounded-lg bg-white p-4 shadow-xl">
        ...
        {isGamesLoading && <p>게임 정보를 불러오는 중...</p>}

        {isGamesError && <p>게임 목록을 불러오지 못했습니다.</p>}

        {!isGamesLoading && !isGamesError && (
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            {games.map((game) => (
              <div key={game.id} className="rounded border p-3">
                ...
                <p>⭐ 평점: {game.rating || "정보 없음"}</p>
                <p>📅 출시일: {game.released || "정보 없음"}</p>

                <button
                  onClick={() => setSelectedGameId(game.id)}
                  className="mt-3 rounded bg-blue-500 px-3 py-2 text-white hover:bg-blue-700"
                >
                  상세 정보 보기
                </button>
              </div>
